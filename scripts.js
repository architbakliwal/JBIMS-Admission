				...
				success:function(responseText, statusText, xhr, $form){ 
					...
					if($( "#section_payment" ).valid()) {
				        ...
				        if ($('input[name=paymentmode]:checked', '#section_payment').val() == 'ddbanktransfer') {
				        	$('#demanddraft-container').css('display', 'block');
				        	window.open('http://jbims.edu/admission/admin/ddredirect.php', '_self');
				        }
				        else{
				        	$('#demanddraft-container').css('display', 'none');
	    	...
	    	jQuery('#section_submit').ajaxSubmit({
	    		...
	    		success: function(responseText) {
	    			...
	    			if(responseText == 'success') {
	    				window.open('http://jbims.edu/admission/admin/done.php', '_self');
	    			} else {
	    			}
	    		}
	    	});
		    ...
		    success:function(data){
		    	...
		    	if(isValid(data)) {
			    	...
			    	if(isValid(applicantdata[12])){
			    		var programenrolled = applicantdata[12].programenrolled.substring(1);
			    		$('#coursesapplied').html("<b>Courses applied for : " + programenrolled + "</b>");
			    	}
		    	}
