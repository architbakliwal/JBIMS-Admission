<?php
	...
	if(!CSRF::check('update-username-form')){
	    ...
	    $usersearch = mysql_query("SELECT * FROM ".$mysqltable_name_1." WHERE application_id = '".mysql_real_escape_string($update_finalusername)."'");
		...
		if($userquery && $userquery['login_system_registrations_user_id'] != $_SESSION['userLogin']){
			...
			$update1 = "UPDATE ".$mysqltable_name_1." SET application_id = '".mysql_real_escape_string($update_finalusername)."' WHERE login_system_registrations_user_id = '".mysql_real_escape_string($_SESSION['userLogin'])."'";
