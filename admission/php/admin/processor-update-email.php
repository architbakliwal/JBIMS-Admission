<?php
	...
	if(!CSRF::check('update-email-form')){
	    ...
	    $emailsearch = mysql_query("SELECT * FROM ".$mysqltable_name_1." WHERE email_id = '".mysql_real_escape_string($update_finalemail)."'");
	    ...
	    if($emailquery && $emailquery['login_system_registrations_user_id'] != $_SESSION['userLogin']){
			...
			$update1 = "UPDATE ".$mysqltable_name_1." SET email_id = '".mysql_real_escape_string($update_finalemail)."' WHERE login_system_registrations_user_id = '".mysql_real_escape_string($_SESSION['userLogin'])."'";
