		...
		if($resultexpire == 1){
			...
			if($result == 1){
				...
				if(!CSRF::check('newpassword-form')){
					...
					$update = "UPDATE ".$mysqltable_name_1." SET password = '".mysql_real_escape_string($newpassword)."', salt = '".mysql_real_escape_string($finalsalt)."' WHERE email_id = '".mysql_real_escape_string($finaluseremail)."'";
