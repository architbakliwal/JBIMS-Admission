...
if ( !CSRF::check( 'forgot-form' ) ) {
	...
	$searchid = mysql_query( "SELECT login_system_registrations_user_id FROM ".$mysqltable_name_1." WHERE email_id = '".mysql_real_escape_string( $finaluseremail )."'" );
	...
	if ( $SMTP == true ) {
		if ( $mysql == true ) {
			if ( $datetime > $blockResult['login_system_forgot_password_blocked_time'] ) {
				$search = mysql_query( "SELECT application_id, f_name, l_name FROM ".$mysqltable_name_1." WHERE email_id = '".mysql_real_escape_string( $finaluseremail )."'" );
